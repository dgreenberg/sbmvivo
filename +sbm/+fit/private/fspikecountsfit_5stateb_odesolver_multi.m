function [states, states_eq] = fspikecountsfit_5stateb_odesolver_multi( ...
...
if strcmpi(odemethod, 'backwardeuler') && usemex
    ...
    if nbuffers == 2
        ...
        fspikecountsfit_5state_odesolver_backwardeuler_mainloop_5_2( ...
            spikecounts, A, states_eq, kon', koff', kon_B', koff_B', Btot', g, c0, niter_newton, states, subseglist, stepsize);
        
    else
        
        fspikecountsfit_5state_odesolver_backwardeuler_mainloop( ...
            spikecounts, A, states_eq, kon', koff', kon_B', koff_B', Btot', g, c0, niter_newton, states, subseglist, stepsize);
    ...
    for ii = 1:numel(subseglist)
        ...
        dfunc = @(t, y) ratefunc_5s(y, koff(jj, :)', kon(jj, :)', koff_B(jj, :)', kon_B(jj, :)', g(jj), c0(jj), Btot(jj, :)');
        
        if strcmpi(odemethod, '45')
            
            odeopts = odeset('NonNegative', (1:size(states_eq, 1))');
            odefunc = @(tlist, y0) ode45(dfunc, tlist, y0, odeopts);
            
        elseif strcmpi(odemethod, '23')
            
            odeopts = odeset('NonNegative', (1:size(states_eq, 1))');
            odefunc = @(tlist, y0) ode23(dfunc, tlist, y0, odeopts);
            
        else
            
            odeopts = odeset('reltol', 1e-3);
            
            Jfunc = @(t, y) ratefuncjac_5s(y, koff(jj, :)', kon(jj, :)', koff_B(jj, :)', kon_B(jj, :)', g(jj), Btot(jj, :)');
...
function dy_dt = ratefunc_5s(y, koff, kon, koff_B, kon_B, g, c0, Btot)
...
dc = -sum(r) - sum(db) - (c - c0) * g;  % free calcium
dy_dt = [dc; ds; db];


function J = ratefuncjac_5s(y, koff, kon, koff_B, kon_B, g, Btot)
nbindingsteps = numel(kon);
c = y(1); s = y(2:2 + nbindingsteps); b = y(2 + nbindingsteps + 1:end);
ns = numel(s);
nb = numel(b);

d2c_dtdc = -g - kon' * s(1:ns - 1) - kon_B' * (Btot - b);
