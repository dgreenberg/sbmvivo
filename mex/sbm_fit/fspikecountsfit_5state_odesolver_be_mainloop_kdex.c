 ...
 * mex fspikecountsfit_5state_odesolver_be_mainloop_kdex.c CFLAGS="\$CFLAGS -fopenmp" LDFLAGS="\$LDFLAGS -fopenmp"
 *
 * compilation with MSVC on windows:
 * mex fspikecountsfit_5state_odesolver_be_mainloop_kdex.c  COMPFLAGS="/openmp $COMPFLAGS"
 */
...
#define VAR_maxex prhs[8]
#define VAR_c0 prhs[9]
#define VAR_kd_ex prhs[10]
#define VAR_niter prhs[11]
#define VAR_states prhs[12]
#define VAR_subseglist prhs[13]
#define VAR_stepsize prhs[14]

#define NINPUTS_MIN 15
#define NINPUTS_MAX 15
#define NOUTPUTS 0
...
void odesolve_5sb_backward(int T, double stepsize,
        ...
        int nbuffers, int n_ex, double *b, double *bprev, double *db_dt,
        ...
        double *maxex, double *kd_ex, double c0,
        ...
        )
{
    ...
    double c, cprev, rate, dc_dt, M_c_c, z_c, a, denom, c_leak;
    
    c_leak = 0.0;
    for (j = 0; j < n_ex; j++) {
        
        c_leak += maxex[j] * c0 / (c0 + kd_ex[j]); /* inward leak current */
    ...
    for (k = 0; k < T; k++) {
        ...
        for (i = 0; i < niter; i++) { /* Newton iterations */
            ...
            for (j = 0; j < n_ex; j++) {
                ...
                dc_dt -= maxex[j] * c / (c + kd_ex[j]);
                M_c_c += stepsize * maxex[j] * kd_ex[j] / ((c + kd_ex[j]) * (c + kd_ex[j]));
...
int calc_temparray_size(ns, nbuffers, nseg) {
    ...
    nbytes_needed =
            ...
            nseg * sizeof(int) + /* T */
            10 * sizeof(double *) * nseg; /* states, spikecounts, initial_state, kon, koff, Btot, kon_B, koff_B, maxex, kd_ex */
...
{
    int nbindingsteps, nbuffers, nsegments_used, nsegments_available, r, ii, nbytes_needed, ns, offset, n_ex;
    double *subsegs;
    char *tmpdata;
    double **states, **spikecounts; /* outputs passed as inputs from matlab */
    double **initial_state, **kon, **koff, **Btot, **kon_B, **koff_B, **maxex, **kd_ex; /* vector parameters */
    double *c0, *A; /* scalar parameters */
    ...
    kd_ex = (double **) (tmpdata + offset);
    offset += nsegments_available * sizeof(double *);
    
    maxex = (double **) (tmpdata + offset);
    offset += nsegments_available * sizeof(double *);
    ...
    if (maxex == NULL || kd_ex == NULL || c0 == NULL || A == NULL) {
    ...
    if (
            ...
            RetrieveDoubleArrayCols(VAR_kd_ex, kd_ex, nsegments_available) < 0 ||
            RetrieveDoubleArrayCols(VAR_maxex, maxex, nsegments_available) < 0
            ) {
...
#pragma omp parallel for
    for (r = 0; r < nsegments_used; r++) { /* solve the ODEs */
        ...
        odesolve_5sb_backward(T[ii], stepsize[ii],
                ...
                nbuffers, n_ex, b + ii * nbuffers, bprev + ii * nbuffers, db_dt + ii * nbuffers,
