function [x,g,xn,gg,qik] = kmeanhar(d,k,l,e,x0)
...
for j=1:l
    ...
    xsq = sum(x .^ 2, 2);
    py = bsxfun(@plus, xsq, dsqt) - 2 * x * dt;
    ...
    pr=ones(k,n);                   % leaving others at 1
    ...
    x = xs ./ qk(:,wp);
    x(qk == 0, :) = ones(sum(qk == 0), 1) * dmean; %avoid NaNs when we have too many clusters
    if g1-g<=th*g1
        ss=ss-1;
