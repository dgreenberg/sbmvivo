...
if ~isfield(P,'b'),     P.b     = dataquantile(F, 0.45);        end % need to check how well this works with spatial filtering
if ~isfield(P,'sig'),   P.sig   = mean(datamad(F, true) * 1.4826);       end
...
function y = dataquantile(x, p)

s = sort(x);
i = max(1, round(numel(x) * p));
y = s(i);

end


function y = datamad(x, usemedian)

if ~exist('usemedian', 'var') || ~usemedian
