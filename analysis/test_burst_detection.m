...
bursts_starts  = st_true(diff([-inf; st_true(:)]) > 2 * tol & diff([st_true(:); inf]) < tol); % AP times that have no preceeding APs within 2*tol, but at least one subsequent AP with tol
